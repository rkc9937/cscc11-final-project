...
def create_severity(df):
    """Create severity weights for different crime categories based on average length of incarceration"""
    ...
    SEVERITY_WEIGHTS = {
        'Assault': None,
        'Robbery': None,
        'Break and Enter': None,  # Must match MCI_CATEGORY in CSV
        'Auto Theft': None,
        'Theft Over': None,
    }
    ...
    SEVERITY_WEIGHTS['Break and Enter'] = avg_length_burglary.mean() * bdf_percentage_guilty

    print('Severity weight for break and enter:', SEVERITY_WEIGHTS['Break and Enter'])
...
def load_severity_weights(filepath='./data/weights/severity_weights.pkl'):
    """Load severity weights from pickle file."""
    with open(filepath, 'rb') as f:
        severity_weights = pickle.load(f)
